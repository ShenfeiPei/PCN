...
def argsort(a, b=0):
    if isinstance(b, int):
        ...
        assert len(b) == len(a)
        two_col = np.concatenate((a.reshape(-1, 1), b.reshape(-1, 1)), axis=1)
        tmp_data = list(tuple(map(tuple, two_col)))
        tosort = np.array(tmp_data, dtype=[("dist", "<f8"), ("ind", "<f8")])
        ret = np.argsort(tosort, order=["dist", "ind"])
